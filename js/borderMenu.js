...
(function() {
	...
	function init() {
		...
		var menu = document.getElementById( 'k-menu' ),
			trigger = menu.querySelector( 'a.k-menu-trigger' ),
			// triggerPlay only for demo 6
			triggerPlay = document.querySelector( 'a.k-menu-trigger-out' ),
			// event type (if mobile use touch events)
			eventtype = mobilecheck() ? 'touchstart' : 'click',
			resetMenu = function() {
				classie.remove( menu, 'k-menu-open' );
				classie.add( menu, 'k-menu-close' );
			},
		...
		trigger.addEventListener( eventtype, function( ev ) {
			...
			if( classie.has( menu, 'k-menu-open' ) ) {
				resetMenu();
			}
			else {
				classie.remove( menu, 'k-menu-close' );
				classie.add( menu, 'k-menu-open' );
